...
dynamically as they are found. Only one search can be active at a time.
FindResults are displayed in a scrolling list within a results tab panel.
<P>
...
<P>
The starting folder of the search (the search base) is displayed at the
top of the accessory panel. The search base dsiplay will not change while
a search is running. Thes search base display will change to reflect the
	...
	*/
	...
	public void goTo (File f) {
		...
		if (f.getParent() == null)
			...
			parentFolder = new File(f.getParent());
// UD		File parentFolder = f.getParentFile();

		if (parentFolder != null) chooser.setCurrentDirectory(parentFolder);

		// Nullify the current selection, if any. Why is this necessary?
    ...
    /**
		...
		matching each filter in the <b>filters</b> array. To interrupt set <b>killFind</b>
		to true. Also stops when number of search hits (matches) equals <b>maxMatches</b>.
		<P>
		<b>Note:</b> Convert this to a nonrecursive search algorithm on systems
		where stack space might be limited and/or the search hierarchy might be very deep.
...
// UD		folder = base.getParentFile();

		if (folder == null) return;

// UD		File[] files = folder.listFiles();
		...
		for (int i=0; i<files.length; i++) {
			...
			if (accept(f, filters)) {
				matches++;
				searchTabs.addFoundFile(f);
			}
			updateProgress();
			if (killFind) return;
			Thread.currentThread().sleep(0);

			if (f.isDirectory()) runFind(f, filters);
    ...
    /**
		...
		This really should be implemented as a property change listener.
    ...
    /**
		...
		progress variables and retrieves the search filter array from the options panel.
	...
	*/
	class FindControls extends JPanel {
		...
		/**
			...
			starting and stopping a search and a simple display for search progress.
		*/
	...
	*/
	class FindResults extends JPanel {
		...
		/**
			...
			be instructed to select the file in the parent JFileChooser's item display.
		*/
		FindResults () {
			super();
			setLayout(new BorderLayout());
		...
		*/
		class FindResultsCellRenderer extends JLabel implements ListCellRenderer {
			...
			public Component getListCellRendererComponent (JList list, Object value,
				...
				if (index == -1) {
					// This shouldn't happen since we won't be using this renderer in a combo box
	...
	*/
	protected long endDateToTime (String s) {
		...
		if (d == null) {
			...
			// Valid date. Now add 24 hours to make sure that the date is inclusive
	...
	class ContentFilter implements FindFilter {
		...
		public boolean accept (File f, FindProgressCallback callback) {
			...
			try {
				...
				while((c = in.read()) != -1) {
					...
					// Report progress to search monitor and abort if method returns false.
					if (callback != null) {
		...
		*/
		...
		/**
			...
			a LocatedStream exception for every occurrence of the byte array.
		*/
		class LocatorStream extends OutputStream {
			...
			*/
			...
			/**
				...
				Throw a MatchMadeException when the bytes written equals the match array.
