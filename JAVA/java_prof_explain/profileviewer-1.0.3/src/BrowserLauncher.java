 ...
 *
 * Ulf Dittmer 3/2006: Adapted to Mac Java 1.4 and up.
 * Ulf Dittmer 6/2009: Replaced Netscape by Firefox (which uses the same command line parameters)
 */
...
public class BrowserLauncher {
	...
	/** JVM constant for Java 1.4 */
	private static final int MRJ_JAVA14 = 5;
    ...
    /**
     * The third parameter for Runtime.exec() on Windows.  This is a "title" parameter that
	 * the command line expects.  Setting this parameter allows URLs containing spaces to work.
     */
    private static final String THIRD_WINDOWS_PARAMETER = "\"\"";

	/**
	 * The shell parameters for Firefox that opens a given URL in an already-open copy of Firefox
	...
	static {
		...
		if (osName.startsWith("Mac OS")) {
			String javaVersion = System.getProperty("java.version");	// something like "1.4.2_09 "
			String mrjVersion = System.getProperty("mrj.version");
			String majorMRJVersion = mrjVersion.substring(0, 3);
//System.out.println("mrjVersion="+mrjVersion);
//System.out.println("javaVersion="+javaVersion);
			try {
				...
				if (version == 2) {
					...
					jvm = MRJ_3_0;
				} else if (version >= 3.1 && version < 4) {
					// Java 1.3 is the last to use this numbering scheme; on 10.3 it prints "3.3.3"
					// Java 1.4 has something like "233" in there
					jvm = MRJ_3_1;
				} else if ((javaVersion != null) && (javaVersion.length() > 0)) {
					// Assume that all versions of MRJ later than JDK 1.3 work the same.
					jvm = MRJ_JAVA14;
				} else {
		...
		switch (jvm) {
			case MRJ_2_0:
				try {
					...
					// There's probably a way to do this better; if anyone has any ideas, please let me know.
					return appleEvent;
				} catch (IllegalAccessException iae) {
					browser = null;
					errorMessage = iae.getMessage();
		...
		switch (jvm) {
		    ...
		    case WINDOWS_9x:
				...
				Process process = Runtime.getRuntime().exec(new String[] { (String) browser,
						FIRST_WINDOWS_PARAMETER, SECOND_WINDOWS_PARAMETER, THIRD_WINDOWS_PARAMETER, '"' + url + '"' });
			...
			case OTHER:
				...
				process = Runtime.getRuntime().exec(new String[] { (String) browser,
						FIREFOX_REMOTE_PARAMETER, FIREFOX_OPEN_PARAMETER_START + url + FIREFOX_OPEN_PARAMETER_END });
				try {
