...
方法2:$savePath = “./session_save_dir/”;//某些文件也有自动垃圾回收机制，因而当将session文件保存在该类文件夹下时也会有声明周期
$lifeTime = 小时 * 秒;
session_save_path($savePath);
session_set_cookie_params($lifeTime);(相对的函数为session_get_cookie_params())
session_start();
方法3:setcookie() and session_set_cookie_params($lifeTime);设置sessionid存储在cookie的事件




stream_get_contents与file_get_contents函数功能完全相同，只是传入的参数为一个打开的文件资源句柄，而file_get_contents为文件名
...
22、JS的转向函数是history.go()?怎么引入一个外部JS文件?(2分)
...
<script type=”text/javascript” src=”js/js_function.js”></script>
    ...
    语法int mysql_num_rows  ( resource $result  )
...
hits 点击量
答:
create table mesasage(
id int(10) unsigned not null auto_increment,
title varchar(20) not null default '',
content varchar(5000) not null default '',
category_id int(10) unsignde not null default 0,
...
comment_content 回复内容
    ...
    comment_content varchar(500) not nul  default '',
    id int(10) unsigned not null default 0,
    priimary key(comment_id)
    ) engine myisam charset utf8;
...
select title,hits,comment_sum from message left join (select id,sum(id) as comment_sum from comment group by id) as tmp on message.id=tmp.id order by comment_sum; 
...
填空题:
1.在PHP中，当前脚本的名称(不包括路径和查询字符串)记录在预定义变量$_SERVER['PHP_SELF']或者__FILE__中;而链接到当前页面的URL记录在预定义变量$_SERVER['HTTP_REFERER']
中
...
10.一个函数的参数不能是对变量的引用，除非在php.ini中把allow_call_time_pass_reference boolean__设为on.

11.SQL中LEFT JOIN的含义是自然左外链接。如果 tbl_user记录了学生的姓名(name)和学号(ID)，tbl_score记录了学生(有的学生考试以后被开除了，没有其记录)的学号(ID)和考试成绩(score)以及考试科目(subject)，要想打印出各个学生姓名及对应的的各科总成绩，则可以用SQL语句____。
